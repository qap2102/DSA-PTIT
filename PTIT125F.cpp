...
int main(){
	...
	while(getline(cin,s)){
		...
		int siz=s.size();
		int cntY=0, cntN=0, cntP=0, cntA=0;
		for(int i=0;i<siz;++i){
			if(s[i]=='Y') cntY++;
			else if(s[i]=='N') cntN++;
			else if(s[i]=='P') cntP++;
		...
		if(cntA>=ceil((double)siz / 2)) cout<<"need quorum"<<endl;
		
		
		else if(cntY>cntN) cout<<"yes"<<endl;
		else if(cntY<cntN) cout<<"no"<<endl;
		else if(cntY==cntN) cout<<"tie"<<endl;
